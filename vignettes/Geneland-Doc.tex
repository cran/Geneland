%% compile 
%% pdflatex Geneland-Doc; bibtex Geneland-Doc; makeindex Geneland-Doc; pdflatex Geneland-Doc; pdflatex Geneland-Doc
...
%\title{Population genetics analysis using R and the Geneland programs}
...
%% \hspace{4cm} {{\huge \em Technical University of Denmark} \\
%% \hspace{7cm} {{\huge \em Copenhagen, gigu[\`a]imm.dtu.dk}
...
%% Special thanks go to Axel Hille who fulfilled a long-standing need of Geneland users by writing and sharing an R script showing how to 
...
%% {\sc Geneland} is based on the add-on packages \texttt{field, RandomFields, maps,mapproj} and \texttt{snow}. 
%% They have to be also installed as they  do not belong to the R base 
%% distribution. 
%% All packages can be installed at the same time via the command-line 
%% by typing:\\
%%  \texttt{install.packages(c("Geneland","fields","RandomFields","maps","mapproj","snow"))}. \\


%% The use of the graphical interface for parallel computations with  \texttt{snow} requires also 
%% packages  \texttt{Rmpi} or \texttt{rpvm} (to be installed too).\\
...
%\hspace{3cm} \includegraphics[width=9.5cm]{fig/exp_q_vect.pdf}
\end{tabular}
...
%% \begin{figure}[h]
%% \begin{tabular}{c}
%% \vspace{-1.5cm}\hspace{3cm} \includegraphics[width=9.5cm]{fig/exp_q_bis.pdf} \\
%% \hspace{3cm} \includegraphics[width=9.5cm]{fig/exp_q_vect.pdf}
%% \end{tabular}
%% \caption{Examples of spatial variation of expected admixture proportions in presence of two clusters. 
%% Individuals whose proportions are displayed here are assumed to be continuously located along a linear 
...
\begin{figure}[h]
\centerline{\input{fig/dag-final.pdf_t}}
...
through the arguments of .\index{read.table}
...
%% R package\footnote{The script is due to Alex Hille who is warmly thanked for sharing with the Geneland users.}.
...
\subsection[Selecting best run]{How to select a run among several runs performed under the same model options?}
...
\subsection[Selecting best run]{How to select a run among several runs performed under different model options?}
...
The first version of {\sc Geneland}  \cite{Guillot05a,Guillot05b} was prone to this issue which was mostly due 
...
Guillot09i,Guillot10b,Guillot10c}

c      fonction SDOT ligne 2520 commentée par Gilles le 31/10/03
c      pour éviter conflit avec routines d'algèbre linéaire
c     decommentée le 5/02/04 sinon ça compile pas (???)
...
c next line modified by Gilles 
c      IF (xnp-30.) 150,30,30
      IF (xnp-30.  .lt. 0) then 
        go to 150
       else 
          go to 30
...
C
      ...
      g = (n+1)*r
c next line modified by Gilles
c      IF (m-ix) 80,120,100
      if(m - ix .lt. 0) then 
         go to 80 
      else if(m - ix .eq. 0) then 
         go to 120
      else
         go to 100
      endif
...
C
      ...
      w2 = w*w
c next line modified by Gilles
c$$$      IF (alv- (xm*alog(f1/x1)+ (n-m+.5)*alog(z/w)+ (ix-
c$$$     +    m)*alog(w*p/ (x1*q))+ (13860.- (462.- (132.- (99.-
c$$$     +    140./f2)/f2)/f2)/f2)/f1/166320.+ (13860.- (462.- (132.- (99.-
c$$$     +    140./z2)/z2)/z2)/z2)/z/166320.+ (13860.- (462.- (132.- (99.-
c$$$     +    140./x2)/x2)/x2)/x2)/x1/166320.+ (13860.- (462.- (132.- (99.-
c$$$     +    140./w2)/w2)/w2)/w2)/w/166320.)) 170,170,30 
...
c     next lines added by Gilles

c next 5 lines added by Gilles 
      u = 0
      difmuk =0
      fk = 0
      e = 0
...
C
c next line modif by Gilles
c   80 IF (t.LT. (-.7187449)) GO TO 70

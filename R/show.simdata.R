        ...
        for(iloc in loc.grid)
          {
            for(iall in 1:(dataset$allele.numbers[iloc]))
              {
                if(is.na(file.plot.freq.grid)){get(getOption("device"))()
                                          }else
                {
                ...
                for(ipop in 1:dataset$npop)
                  {
                    ...
                    FF[cc==ipop] <- ff[cc==ipop]
                                        ...
                                        #X11()
                 if(is.na(file.plot.freq.indiv)){get(getOption("device"))()
                                          }else
                    postscript(paste(substr(file.plot.freq.indiv,
                                          start=1,
    ...
    if(plot.gen==TRUE)
      {
        ...
        for(iloc in 1:loc.grid)
          {
                                        #X11()
            if(is.na(file.plot.gen)){get(getOption("device"))()
                                     }else
            postscript(paste(substr(file.plot.gen,
                                    start=1,

    ...
    jcf = FALSE, filter.null.alleles = FALSE) 
{
    ...
    if (is.null(coordinates)) {
        if (spatial) {
            ...
            nindiv <- ifelse(ploidy == 2, ncol(genotypes)/2, 
                ncol(genotypes))
    ...
    out.res <- .Fortran(name = "mcmc", PACKAGE = "Geneland", 
        ...
        as.integer(ploidy), as.character(path.mcmc), as.integer(nchar.path), 
        as.single(rate.max), as.single(delta.coord), as.integer(nit), 
        ...
        as.single(t), as.single(ttemp), as.single(u), as.single(utemp), 
        as.integer(c), as.integer(ctemp), as.single(f), as.single(ftemp), 
        as.single(fa), as.single(drift), as.single(drifttemp), 
        as.integer(indcell), as.integer(indcelltemp), as.single(distcell), 
        as.single(distcelltemp), as.single(xlim), as.single(ylim), 
        ...
        as.integer(seed1), as.integer(seed2), as.integer(true.genotypes), 
        as.single(full.cond.y))
    param <- c(paste("ploidy :", ploidy), paste("rate.max :", 
        ...
        freq.model), paste("varnpop :", varnpop), paste("filter.null.alleles :", 
        filter.null.alleles), paste("spatial :", spatial))

...
function (nindiv, coord.indiv, coord.lim = c(0, 1, 0, 1), rate, 
    ...
    npop, seed.coord, seed.tess, seed.freq, give.tess.grid = FALSE, 
    ...
    comp.diff = FALSE, width, plot.pairs.borders = FALSE) 
{
    ...
    if (IBD) {
        ...
        ff <- GaussRF(x = coord.all[1, ], y = coord.all[2, ], 
            grid = FALSE, model = model, param = c(0, 1, 0, beta, 
                gamma), n = sum(allele.numbers) * npop)
        print("End of GaussRF")
    }
    else {
        ff <- matrix(nr = ncol(coord.all), nc = sum(allele.numbers) * 
            npop, data = rnorm(sum(allele.numbers) * npop), byrow = TRUE)
    }
    count <- 1
    for (ipop in 1:npop) {
        for (iloc in 1:nloc) {
    ...
    for (ipop in 1:npop) {
        for (iloc in 1:nloc) {
            for (iall in 1:allele.numbers[iloc]) {
                freq[ipop, , iloc, iall] <- y[ipop, , iloc, iall]/apply(y[ipop, 
                  , iloc, 1:allele.numbers[iloc]], 1, sum)
            }
        }
    }
    print("freq. stored")
    ...
    if (give.freq.grid) {
        freq.grid <- array(data = freq[, -(1:nindiv), , ], dim = c(npop, 
                ...
                for (ipop2 in (ipop1 + 1):npop) {
                  ...
                  if (length(sub2) > 0) {
                    ...
                    dd <- as.matrix(dist(t(coord.indiv[, c(sub1, 
                      sub2)])))
                    diag(dd) <- Inf
                    npairs <- sum(dd[1:nindiv1, (nindiv1 + 1):(nindiv1 + 
                      nindiv2)] < width)
    ...
    res <- list(coord.lim = coord.lim, coord.indiv = t(coord.indiv), 
        ...
        coord.nuclei = coord.nuclei, color.nuclei = color.nuclei, 
        freq.indiv = freq.indiv, gf.indiv = gf.indiv, nearest.nucleus.indiv = nearest.nucleus.indiv, 
        Fst = Fst, Fis = Fis, Dsigma2 = Dsigma2, heter = heter, 
        diff.B = diff.B, diff.W = diff.W)
    if (give.tess.grid | give.freq.grid) {

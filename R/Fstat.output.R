...
function (coordinates = NULL, genotypes, ploidy = 2, burnin = NULL, 
    path.mcmc) 
{
    param <- as.matrix(read.table(file = paste(path.mcmc, "parameters.txt", 
        sep = "")))
    post.param <- as.matrix(read.table(file = paste(path.mcmc, 
        "postprocess.parameters.txt", sep = "")))
    pmpi <- read.table(file = paste(path.mcmc, "modal.pop.indiv.txt", 
        sep = ""))[, 3]
    ...
    npop.est <- order(hist(x = npop[-(1:burnin)], breaks = seq(0.5, 
